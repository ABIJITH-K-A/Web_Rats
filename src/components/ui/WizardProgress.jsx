      ...
      {steps.map((step, idx) => {
        ...
        return (
          <div key={idx} className="flex flex-col items-center gap-2 flex-1 relative group">
            {/* Line */}
            {idx < steps.length - 1 && (
              <div className={`absolute top-[17px] left-1/2 w-full h-[2px] z-0 ${isDone ? 'bg-cyan-primary/50' : 'bg-white/5'}`} />
            )}
            
            {/* Dot */}
            <div className={`w-9 h-9 rounded-full border-2 flex items-center justify-center font-mono text-xs relative z-10 ${
