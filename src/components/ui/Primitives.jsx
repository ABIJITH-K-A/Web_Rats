...
const Button = ({ children, variant = 'primary', className = '', ...props }) => {
  ...
  const variants = {
    primary: 'bg-primary-dark border border-cyan-primary/80 text-cyan-primary',
    outline: 'bg-black/25 border border-cyan-primary/55 text-cyan-primary',
  ...
  return (
    <button
      ref={buttonRef}
      className={`relative isolate overflow-hidden font-bold flex items-center justify-center gap-2 ${isGalaxy || isCardCta || isStartProject ? '' : 'px-8 py-3 rounded-full'} ${variants[variant]} ${className}`}
      {...props}
    >
      {isGalaxy ? (
        <>
